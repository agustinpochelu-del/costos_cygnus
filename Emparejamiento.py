...
if archivo_costos is not None and archivo_detalle is not None:
    ...
    # LIMPIEZA CLAVE: Le sacamos los espacios invisibles a los nombres de las columnas
    df_costos.columns = df_costos.columns.str.strip()
    df_detalle.columns = df_detalle.columns.str.strip()

    # Validación: Chequeamos que las columnas clave existan antes de avanzar
    columnas_faltantes = False
    if 'Medida' not in df_detalle.columns:
        st.error("🚨 No encontré la columna 'Medida' en la planilla de Detalle. Chequeá que el archivo sea el correcto o que el título esté bien escrito.")
        columnas_faltantes = True
    if 'Nombre del Producto' not in df_costos.columns:
        st.error("🚨 No encontré la columna 'Nombre del Producto' en la planilla de Tiendanube.")
        columnas_faltantes = True

    # Solo avanzamos si están todas las columnas
